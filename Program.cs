    ...
    class Program
    {
        ...
        static void ExportReport(string scanDetails, List<string> conclusionLines, string osInfo, string cpuInfo, string scanTime)
        {
            ...
            try
            {
                ...
                // CRITICAL FIX: Ensures the TXT is exported to the folder where the EXE physically resides, 
                // preventing it from being lost in the %TEMP% directory during Single-File extraction.
                string currentDir = Path.GetDirectoryName(Environment.ProcessPath) ?? Environment.CurrentDirectory;
        ...
        public static string GetFriendlyOS()
        {
            ...
            if (!string.IsNullOrEmpty(osName))
            {
                ...
                string fullOsName = string.IsNullOrEmpty(osVersion) ? osName : $"{osName} (Version {osVersion})";

                // Heuristic detection for debloated or custom OS distributions (e.g. AtlasOS, ReviOS).
                string customOSMarker = "";
                if (Directory.Exists(@"C:\Windows\Atlas") || Directory.Exists(@"C:\ProgramData\Atlas") || Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Atlas") != null || Registry.LocalMachine.OpenSubKey(@"SOFTWARE\AtlasOS") != null)
                {
                    customOSMarker = " [AtlasOS Custom Edition]";
                }
                else if (Directory.Exists(@"C:\Windows\Revision") || Directory.Exists(@"C:\ProgramData\Revision") || Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Revision") != null || Registry.LocalMachine.OpenSubKey(@"SOFTWARE\ReviOS") != null)
                {
                    customOSMarker = " [ReviOS Custom Edition]";
                }

                return fullOsName + customOSMarker;
            }
