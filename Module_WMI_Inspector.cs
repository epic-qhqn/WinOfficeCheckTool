...
namespace WinCheckTool
{
    ...
    public class Module_WMI_Inspector : ICheckModule
    {
        ...
        public CheckResult Execute()
        {
            try
            {
                // Ascertain domain join status to validate enterprise volume channel usage.
                bool isDomainJoined = false;
                try
                {
                    using (ManagementObjectSearcher csSearcher = new ManagementObjectSearcher("SELECT PartOfDomain FROM Win32_ComputerSystem"))
                    using (ManagementObjectCollection csCollection = csSearcher.Get())
                    {
                        foreach (ManagementObject cs in csCollection)
                        {
                            isDomainJoined = Convert.ToBoolean(cs["PartOfDomain"]);
                            break;
                        }
                    }
                }
                ...
                string queryString = "SELECT Name, Description, LicenseStatus, KeyManagementServiceMachine, ApplicationId, GracePeriodRemaining FROM SoftwareLicensingProduct WHERE PartialProductKey IS NOT NULL";
                ...
                using (ManagementObjectCollection objCollection = searcher.Get())
                {
                    ...
                    foreach (ManagementObject obj in objCollection)
                    {
                        ...
                        if (status == 1) // Actively licensed iteration.
                        {
                            ...
                            uint gracePeriod = 0;
                            if (obj["GracePeriodRemaining"] != null) uint.TryParse(obj["GracePeriodRemaining"].ToString(), out gracePeriod);

                            string nameLower = channelName.ToLower();
                            string descLower = channelDesc.ToLower();
                            
                            // Check for KMS38 execution where activation grace period is artificially expanded to year 2038.
                            if (gracePeriod > 1000000)
                            {
                                worstResult = new CheckResult { Status = CheckStatus.Danger, Message = LanguageManager.GetString("WMI_KMS38_DETECTED") };
                                break;
                            }
                            ...
                            {
                                // Detects standalone GVLK deployment without valid enterprise domain networking.
                                if (nameLower.Contains("volume:gvlk") || nameLower.Contains("kmsclient"))
                                {
                                    if (!isDomainJoined)
                                    {
                                        worstResult = new CheckResult { Status = CheckStatus.Warning, Message = LanguageManager.GetString("WMI_DOMAIN_ANOMALY") + $" [[({channelName})]]" };
                                        break;
                                    }
                                }
