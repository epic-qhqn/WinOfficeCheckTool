    ...
    public class Module_Heuristics : ICheckModule
    {
        ...
        public CheckResult Execute()
        {
            try
            {
                ...
                // Detects frozen trial period counters and hardcoded KMS servers.
                ...
                using (RegistryKey rootSppKey = Registry.LocalMachine.OpenSubKey(rootSppPath))
                {
                    if (rootSppKey != null)
                    {
                        ...
                        string kmsServer = rootSppKey.GetValue("KeyManagementServiceServer")?.ToString();
                        if (!string.IsNullOrEmpty(kmsServer))
                        {
                            return new CheckResult { Status = CheckStatus.Danger, Message = LanguageManager.GetString("HEUR_REG_KMS") + $" [[{kmsServer}]]" };
                        }
                    }
                }

                // Evaluates Windows Defender Policies for tampering (commonly done by Lite OS or crackers).
                using (RegistryKey defKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Windows Defender"))
                {
                    if (defKey != null && defKey.GetValue("DisableAntiSpyware")?.ToString() == "1")
                    {
                        return new CheckResult { Status = CheckStatus.Warning, Message = LanguageManager.GetString("HEUR_DEFENDER_DISABLED") };
                    }
                    using (RegistryKey rtKey = defKey?.OpenSubKey("Real-Time Protection"))
                    {
                        if (rtKey != null && rtKey.GetValue("DisableRealtimeMonitoring")?.ToString() == "1")
                        {
                            return new CheckResult { Status = CheckStatus.Warning, Message = LanguageManager.GetString("HEUR_DEFENDER_DISABLED") };
                        }
                    }
                ...
                // Inspects Software Protection Service (sppsvc) startup configuration.
                using (RegistryKey srvKey = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\sppsvc"))
                {
                    // 4 signifies 'Disabled' startup mode.
                    if (srvKey != null && srvKey.GetValue("Start")?.ToString() == "4")
                    {
                        return new CheckResult { Status = CheckStatus.Warning, Message = LanguageManager.GetString("HEUR_SPPSVC_DISABLED") };
                    }
                }
