...
namespace WinCheckTool
{
    ...
    public class Module_Office_Inspector : ICheckModule
    {
        ...
        private CheckResult CheckOhookBypass()
        {
            ...
            // Validates Office Resiliency TimeOfLastHeartbeatFailure parameters often hijacked by Ohook (Far future dates like 203x/204x).
            using (RegistryKey resKey = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Office\16.0\Common\Licensing\Resiliency"))
            {
                if (resKey != null)
                {
                    string heartbeat = resKey.GetValue("TimeOfLastHeartbeatFailure")?.ToString();
                    if (!string.IsNullOrEmpty(heartbeat) && (heartbeat.Contains("203") || heartbeat.Contains("204") || heartbeat.Contains("205")))
                    {
                        return new CheckResult { Status = CheckStatus.Danger, Message = LanguageManager.GetString("OFFICE_OHOOK_RESILIENCY") };
                    }
                }
            }

            // Detects anomalous sppc.dll placement and verifies its Digital Signature to catch renamed Ohook payloads.
            using (RegistryKey ctrKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Office\ClickToRun\Configuration"))
            {
                if (ctrKey != null)
                {
                    string installFolder = ctrKey.GetValue("InstallFolder")?.ToString();
                    if (!string.IsNullOrEmpty(installFolder))
                    {
                        string sppcPath = Path.Combine(installFolder, @"root\vfs\System\sppc.dll");
                        if (File.Exists(sppcPath))
                        {
                            if (!IsMicrosoftSigned(sppcPath)) 
                            {
                                return new CheckResult { Status = CheckStatus.Danger, Message = LanguageManager.GetString("OFFICE_SIGNATURE_FAIL") + $" [[{sppcPath}]]" };
                            }
                            return new CheckResult { Status = CheckStatus.Danger, Message = LanguageManager.GetString("HEUR_OHOOK") + $" [[{sppcPath}]]" };
                        }
                    }
                }
            }
